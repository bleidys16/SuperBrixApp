...
| Uso de la IA (25%) | Apps Script llama a NVIDIA NIM (LLM) para clasificar el texto libre en las 6 categorías oficiales del reto. Fallback por palabras clave si la API falla, para no perder la demo. |
...
|---|---|---|---|
| 1. Setup | 0–2h | Los 3 | Crear el Sheet, pegar `Code.gs`, conseguir API key NVIDIA, correr la app en el emulador. |
...
- **Sin wifi/datos en el venue** → el fallback por palabras clave permite seguir
  la demo sin la API de NVIDIA (criterio de IA bajará un poco, pero no se cae
  la demo completa).
- **Cuota de la API de NVIDIA agotada** → mismo fallback.
